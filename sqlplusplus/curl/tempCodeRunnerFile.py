...
def askQueries(self, queryList):
    ...
    for query in queryList:
        ...
        self.sentence_map[negatedPredicate.name] = self.sentence_map.get(
            negatedPredicate.name, []
        ) + [negatedQuery]
        self.timeLimit = time.time() + 40
        try:
            result = self.resolve(
                [negatedPredicate], [False] * (len(self.inputSentences) + 1)
            )
        except:
            result = False
                ...
                if not visited[kb_sentence.sentence_index]:
                    for kbPredicate in kb_sentence.findPredicates(queryPredicateName):
                        ...
                        if canUnify:
                            ...
                            if (
                                kb_sentence.containsVariable()
                                and len(kb_sentence.predicates) > 1
                            ):
...
def performUnification(self, queryPredicate, kbPredicate):
    substitution = {}
            ...
            if kb.isConstant():
                ...
                if not query.isConstant():
                    ...
                    if kb.name not in substitution:
                        ...
                        return False, {}
                    query.unify("Variable", kb.name)  # Update this line
    return True, substitution


def negatePredicate(self, predicate):
    return predicate[1:] if predicate[0] == "~" else "~" + predicate


def negateAntecedent(self, sentence):
    antecedent = sentence[: sentence.find("=>")]
    premise = []
    for predicate in antecedent.split("&"):
        premise.append(self.negatePredicate(predicate))
...
def printOutput(self, filename, results):
    print(results)
...
def main(self):
    inputQueries_, inputSentences_ = self.getInput("input.txt")
    knowledgeBase = KB(inputSentences_)
    knowledgeBase.prepareKB()
    results_ = knowledgeBase.askQueries(inputQueries_)
    self.printOutput("output.txt", results_)
